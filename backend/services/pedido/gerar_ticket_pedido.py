...
class GerarTicketPedidoService:
    def executar(self, id_pedido: int, user_id):
        ...
        return {'ticket_header': f'=== BORA OBRA - TICKET #{pedido.id} ===', 'data_hora': pedido.created_at, 'loja_origem': loja.nome if loja else 'N/A', 'loja_endereco': loja.endereco if loja else '', 'loja_telefone': loja.telefone if loja else '', 'cliente': cliente.nome if cliente else 'N/A', 'tipo_operacao': pedido.tipo, 'itens': itens_ticket, 'forma_pagamento': pedido.forma_pagamento or 'Nao informado', 'endereco_entrega': pedido.endereco_entrega or 'Retirada na loja', 'valor_total': f'R$ {pedido.valor_total:.2f}', 'status': pedido.status, 'observacao': pedido.observacao or 'Sem observacoes'}